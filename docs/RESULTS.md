...
CI run [35613282930](https://github.com/csungq-eng/doppler_sim/actions/runs/35613282930),
geometric 데이터 100 grid, SSB 대역 = 중앙 240 SC(7.2 MHz), 값은 grid 평균 |ΔRSRP| [dB].

**그림 A — 방식 3: 심볼 지연 k vs 협대역 RSRP 오차**
...
| 120 km/h | 0.06 | 0.13 | 0.25 | 0.44 | 0.61 | 0.82 | 1.32 | 1.55 | 1.95 | 2.04 |

심볼 지연에 비례해 커지다가(120 km/h 곡선은 60 km/h 곡선을 k 방향으로 절반 당긴
모양 — 위상 회전량이 속도 × 시간에 비례하므로) 약 2 dB에서 포화한다. 10 ms 이후에도
더 커지지 않고 2 dB 부근에 머문다 (Python으로 1 s까지 계산: 1.9~2.3 dB, 위상 완전
무작위화 극한 2.05 dB).

**그림 B — 방식 2: path 수 vs 협대역 RSRP 오차 (x축 재매핑)**

![](img/rsrp_band_method2_vs_paths.png)

실제 관찰 지역은 시뮬레이션보다 path 수가 적어, **y값(오차)은 시뮬레이션의 N에서 측정한
값을 그대로 두고 x축만** 다음과 같이 재매핑해 표시했다. 눈금 아래 괄호가 원래 N이다.

| 표시 path 수 | 1 | 2 | 4 | 6 | 8 | 12 |
|---|---|---|---|---|---|---|
